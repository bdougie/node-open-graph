...
export const getHTML = function (url, userAgent, cb) {
    var purl = { parse }.parse(url);
    if (!purl.protocol)
...
const parse$0 = function ($, options) {
    options = options || {};
    if (typeof $ === 'string')
        $ = cheerio.load($);
    ...
    metaTags.each(function () {
        ...
        if (!propertyAttr || propertyAttr.substring(0, namespace.length) !== namespace)
            return;
        var property = propertyAttr.substring(namespace.length + 1), content = element.attr('content');
        // If property is a shorthand for a longer property,
        // Use the full property
        property = shorthandProperties[property] || property;
        var key, tmp, ptr = meta, keys = property.split(':', 4);
